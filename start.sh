...
#   1. MetaTrader 5      (Wine, DISPLAY=:0, minimized)
#   2. real_bot.py       (AI trading loop)
#   3. dashboard.py      (Streamlit UI  →  http://localhost:8501)
#
# The watchdog monitors all three processes and restarts any that crash.
...
# Use the real X display so MT5/Wine can render normally without Xvfb.
export DISPLAY=:0
...
echo -e "    ${CYAN}1${RESET}  MetaTrader 5     (Wine, DISPLAY=:0, starts minimized)"
echo -e "    ${CYAN}2${RESET}  real_bot.py      (AI trading loop)"
echo -e "    ${CYAN}3${RESET}  dashboard.py     (Streamlit UI)"
echo ""
